...
function App() {
  ...
  /*
   * 받은 문제에서는 이 버튼이 "그 문제를 그만두고 정답을 새로 뽑아 새 판을 까는" 일을 한다.
   * 하는 일이 달라졌으니 글자도 달라야 한다. "다시하기"라고 적혀 있으면
   * 같은 문제를 처음부터 다시 푸는 줄 알고 누르게 된다.
   *
   * "기존 게임으로"나 "평소 게임으로"라고 쓰지 않는다. 돌아갈 판이 없기 때문이다.
   * 링크를 열 때 이미 그 문제로 판이 깔렸으므로 하던 게임은 남아 있지 않고,
   * 이 버튼은 정답을 새로 뽑는다(handleRestart가 sharedAnswer를 null로 넘긴다).
   * "돌아간다"고 적으면 아까 하던 판이 살아 있는 줄 알고 누르게 된다.
   *
   * 확인 창도 "친구가 낸 문제를 그만둘까요?" / "그만두기"라고 묻는다.
   * 버튼과 질문과 답이 전부 '그만둔다'는 같은 말로 이어져야 한다.
   *
