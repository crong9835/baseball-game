 ...
 *
 * 예전에는 "S:1 B:1 OUT:1"처럼 적었지만 야구 규칙을 모르면 읽을 수 없어서 한글로 바꿨다.
 * 0개인 것은 아예 적지 않는다. 0까지 늘어놓으면 정작 봐야 할 숫자가 묻힌다.
 *
 * 문자열 하나로 만들지 않고 조각으로 나누는 이유는 스트라이크와 볼에 서로 다른 색을
 * 입혀야 하기 때문이다.
 */
 ...
 *
 * 초보 모드가 꺼져 있으면 색 클래스를 아예 붙이지 않아 전부 같은 색으로 보인다.
 */
    ...
    <ul className={styles.historyList}>
      {history.map((record) => (
        <li key={record.attemptNumber} className={styles.historyItem}>
          ...
          {/*
            숫자를 이어 붙여 문자열 하나로 만들면 한 자리만 색을 다르게 할 수 없다.
            그래서 자리마다 span을 따로 그리고, 같은 자리의 판정으로 색을 정한다.
            (key로 position을 써도 되는 이유: 한 기록 안의 자리들은 순서가 바뀌지 않는다)
          */}
