...
`PixelBatter`만 다른 열 개와 성질이 다릅니다. 값을 받지도, 알리지도 않는 그림 한 장이고 `StartScreen` 안에서만 씁니다. 그래도 파일을 나눈 이유는 '파일을 나누는 기준' 셋에 다 걸리기 때문입니다 — 도트 그림을 고치는 일과 시작 화면 배치를 고치는 일은 서로 안 읽어도 됩니다.
...
**게임 화면의 제목(`숫자 야구`)을 누르면 시작 화면으로 나갑니다.** 오락기에서 타이틀 화면으로 돌아가는 것과 같습니다. `useScreen`은 화면 이름만 갖고 있어서 게임 state에 손댈 방법이 아예 없고, 그래서 **하던 게임이 지워지지 않습니다.** 잃을 것이 없으니 확인 창도 띄우지 않습니다.
...
**시작 버튼 글자는 세 갈래고 보는 순서가 정해져 있습니다** (`StartScreen`). 기본은 `시작하기`, 받은 문제면 `친구 문제 풀기`, **하던 게임이 있으면(`attemptCount > 0`) `게임으로 돌아가기`가 이깁니다.** 앞의 둘은 이제부터 시작한다는 말이라, 기록이 쌓여 있는데 그렇게 적으면 누르면 기록이 지워지는 줄 알고 못 누릅니다.

여기서는 `돌아가기`라고 적어도 됩니다 — 실제로 돌아갈 게임이 남아 있습니다. 받은 문제를 그만두는 버튼에 그 말을 쓰지 말라는 것은 그쪽에 돌아갈 것이 없기 때문입니다.
...
**네 장을 가로로 이어 붙인 뒤 `steps(4)`로 한 칸씩 밀어서 움직입니다.** 그래서 `PixelBatter.module.css`의 `400%`·`25%`·`steps(4)` 세 곳이 그림 장수와 짝입니다. **CSS는 배열 길이를 읽을 방법이 없으므로 장수를 늘리면 세 곳을 같이 고쳐야 합니다.** 안 고치면 마지막 장이 창문 밖에 남아 영영 안 보입니다.
...
`useScreen`이 가진 것은 `screen` 하나뿐입니다. **여기에 게임 값을 넣지 마세요.** 화면을 바꿔도 하던 게임이 그대로 남는 것은 이 훅이 게임 state에 손댈 방법이 아예 없기 때문입니다.
...
체크박스 설명에 `· 바꾸면 새 게임`을 적어둔 것도 같은 이유입니다. 눌렀을 때 하던 게임이 사라지는데 글자에 안 적혀 있으면 눌러보고 나서야 알게 됩니다.
...
**단, 받은 문제를 그만둘 때(`LEAVE_SHARED_PUZZLE`)는 기록이 0개여도 묻습니다.** 잃는 것이 기록이 아니라 문제 자체이기 때문입니다. 링크는 읽자마자 `removePuzzleFromLink()`가 주소창에서 지우고 뒤로가기 기록도 남기지 않으므로, 첫 시도 전에 잘못 눌러 나가면 친구에게 링크를 다시 받는 수밖에 없습니다. 그래서 `requestNewGame`이 `hasProgressToLose || isLeavingSharedPuzzle`로 판단합니다.
...
**`평소 게임으로`·`기존 게임으로`처럼 "돌아간다"는 말을 쓰지 마세요.** 돌아갈 판이 없습니다. 링크를 열 때 이미 그 문제로 판이 깔려서 하던 게임은 남아 있지 않고, 이 버튼은 정답을 새로 뽑습니다. 돌아간다고 적으면 아까 하던 판이 살아 있는 줄 알고 누르게 됩니다.
