...
- **커밋은 허락을 받고 합니다.** 단계가 끝나면 무슨 작업을 했는지 쓰고, 소유자가 하라고 할 때까지 커밋하지 마세요. `push`도 마찬가지로 명시적으로 요청받았을 때만 합니다

  이 프로젝트의 커밋 메시지는 "왜 그렇게 했는지"를 남기는 기록입니다. 소유자가 그 설명을 읽고 자기 생각과 맞는지 확인한 뒤에 기록으로 굳어져야 합니다. 먼저 커밋해 놓고 알리면, 설명이 틀렸을 때 되돌리는 일부터 해야 합니다
