...
컴포넌트는 `DifficultySelector`, `GuessInput`, `NumberPad`, `SettingToggle`, `ResultBanner`, `HistoryList`, `ConfirmDialog`, `PuzzleComposer`, `StartScreen`, `HelpScreen`, `PixelBatter` 열한 개입니다.

`PixelBatter`만 다른 열 개와 성질이 다릅니다. 값을 받지도, 알리지도 않는 그림 한 장이고 `StartScreen` 안에서만 씁니다.
...
**다만 `handleOpen`에서는 `copyResult`를 비웁니다.** 화면 안에서 값을 바꾸는 조작은 링크를 달라지게 해서 비교만으로 걸러지지만, 화면을 닫아둔 사이에 사용자가 다른 것을 복사한 것은 링크를 바꾸지 않습니다. 클립보드는 이 앱 바깥에 있어서 들여다볼 방법이 없습니다. 그래서 같은 문제를 다시 만들면 누르지도 않은 "복사했습니다"가 떠서, 그 말을 믿고 엉뚱한 글자를 친구에게 보내게 됩니다.
